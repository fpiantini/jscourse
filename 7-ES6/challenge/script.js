...
It's a very small town, so right now there are only 3 parks
and 4 streets. All parks and streets have a name and a build
year.

At an end-of-year meeting, your boss wants a final report with
the following:
1. Tree density of each park in the town (formula: number of
trees/park area)
...
// -----------------------------------------
class Park extends TopoElement {
...
// -----------------------------------------
class Street extends TopoElement {
...
function average(arr) {
  //let average = 0;
  //arr.forEach(cur => average += cur/arr.length);
  //return average;
  return arr.reduce((prev, cur, index) => prev+cur/arr.length, 0);
}

// this is a quite a duplication, but is here to
...
// ---------------------------------------------------------
let streets = [], parks = [];
parks.push(new Park('Green park', 1964, 2, 800));
parks.push(new Park('Oak park', 1980, 7, 2000));
parks.push(new Park('National park', 1800, 10, 20000));
streets.push(new Street('4th street', 1964, 3000));
streets.push(new Street('Evergreen street', 1964, 1500, 'tiny'));
streets.push(new Street('Ocean avenue', 1980, 6000, 'big'));
streets.push(new Street('Sunset boulevard', 2006, 25000, 'huge'));
...
function reportParks(p) {
  ...
  // tree density
  p.forEach(cur => console.log(`${cur.name} has a tree density of ${cur.treeDensity()} trees/Km2`));
  
  // average parks age
  console.log(`Average parks age: ${average(p.map(cur => cur.age()))} years`);
  
  // if the park has more than 1.000 trees
  p.forEach(cur => {if (cur.numberOfTrees > 1000) console.log(`${cur.name} has more than 1.000 trees`)});
}
...
function reportStreets(s) {
  ...
  // total and average streets length
  // with example of destructuring
  const [tot, ave] = totalAndAverage(s.map(cur => cur.length));
  console.log(`Total streets lenght: ${tot} m`);
  console.log(`Average streets lenght: ${ave} m`);
  
  // street size classification
  s.forEach(cur => console.log(`${cur.name} has a size classification of ${cur.size}`));
